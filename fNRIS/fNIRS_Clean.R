#CLEAN FUNCTION FOR fNIRS
##CLEANING SF TXT FILES & POINT COMPUTATION

date <- function(file_name){
  return(as.Date(substr(file_name, 13, 21), format="%Y-%m-%d"))
}
...
date_hour<-function(file_name,clean=FALSE){
  if(clean==TRUE){
    ...
    return(as.POSIXct(str_sub(file_name,13,-5),format="%Y-%m-%d_%H-%M-%S"))
  }
}
...
ID<- function(file_name,clean=FALSE){
  if(clean==TRUE){
    ...
    return(toupper(substr(file_name, 6,11)))
  }
}
...
compute_points<-function(file_to_read, point_only=FALSE,press=FALSE,prct=FALSE){# if df_return = False, return the total score
  e1=file_to_read$e1
...
read_APM_ScM<-function(files_APM_ScM){
  df_APM_ScM=foreach(i=1:length(files_APM_ScM),.combine=rbind)%do%{
    ...
    list(Date=file_APM_ScM$Date[1],Session=file_APM_ScM$Session[1],Pseudo=file_APM_ScM$Pseudo[1]
         ,ScoresMin=file_APM_ScM$ScoresMin
         #,FlightScore=file_APM_ScM$FlightScore,BonusScore=file_APM_ScM$BonusScore
         #,MineScore=file_APM_ScM$MineScore,FortressScore=file_APM_ScM$FortressScore
         ,APM=file_APM_ScM$APM,TotalScore=file_APM_ScM$TotalScore[1],Flight=file_APM_ScM$Flight[1],Bonus=file_APM_ScM$Bonus[1],Mine=file_APM_ScM$Mine[1]
         ,Fortress=file_APM_ScM$Fortress[1],MeanScoreMin=round(mean(unlist(file_APM_ScM$ScoresMin)),0),MeanAPM=round(mean(unlist(file_APM_ScM$APM)),0))  
  }
  ...
  df_APM_ScM$MeanScoreMin=unlist(df_APM_ScM$MeanScoreMin)
  df_APM_ScM$MeanAPM=unlist(df_APM_ScM$MeanAPM)
...
get_day_session<-function(file_to_date,path_to_date,c=FALSE){
  ...
  if(length(sublist)==11){
    ...
    return(switch(i,"P1","P2","P3","D04","D05","D14"))
  }
}
...
read_final_Score<-function(files_data,path_clean){
...
APM_ScM_plot<-function(str_pseudo,df){
  df_plot=data.frame(T=c(1:540),ScM=unlist(df$ScoresMin[df$Pseudo==str_pseudo]),APM=unlist(df$APM[df$Pseudo==str_pseudo]))
  APM_plot=ggplot(df_plot,aes(T,APM))+geom_line()+theme_classic2()
  ScM_plot=ggplot(df_plot,aes(T,ScM))+geom_line()+theme_classic2()
  figure=ggarrange(APM_plot,ScM_plot,labels=c("A","B"),ncol=1,nrow=2,align="v")
  figure=annotate_figure(figure,fig.lab=str_pseudo,fig.lab.pos="bottom.right")
  return(figure)
}

ZScores_Yeo<-function(data){
  #Yeo Transform
  data$Flight_YeoJ=yeojohnson(data$Flight)$x.t
  data$Bonus_YeoJ=yeojohnson(data$Bonus)$x.t
  data$Mine_YeoJ=yeojohnson(data$Mine,standardize = TRUE)$x.t
  data$Fortress_YeoJ=yeojohnson(data$Fortress,standardize = TRUE)$x.t
  #ZSubScore
  data$ZFlight=scale(data$Flight_YeoJ)
  data$ZMine=scale(data$Mine_YeoJ)
  data$ZBonus=scale(data$Bonus_YeoJ)
  data$ZFortress=scale(data$Fortress_YeoJ)
  data$ZMean=rowMeans(subset(data,select=c("ZMine","ZFortress","ZBonus","ZFlight")))
  data=data[,!(colnames(data) %in% c("Flight_YeoJ","Bonus_YeoJ","Mine_YeoJ","Fortress_YeoJ"))]
}
