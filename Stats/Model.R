...
plot_log=ggplot(gen_data_noJ14,aes(log(D),Zscore))+geom_point()+theme_classic2()+annotate("text",label=eq,x=0.5,y=2)+
  geom_abline(intercept=coef(fit_all)[1],slope=coef(fit_all)[2],col="blue")+stat_summary(geom="point",col="red",fun="mean",size=3)+labs(x="log(Day)")

plot_logreg=ggplot(gen_data_noJ14,aes(D,Zscore))+geom_point()+theme_classic2()+annotate("text",label=paste("Equation :","y~log(x)"),x=3,y=2)+
  stat_smooth(method=lm,formula=y~log(x))+stat_summary(geom="point",col="red",fun="mean",size=3)+labs(x="Day")+
  scale_x_continuous(breaks=1:11)
...
logreg_pseudo=do(by_pseudo_noJ14,tidy(lm(Zscore~log(D),data=.)))
